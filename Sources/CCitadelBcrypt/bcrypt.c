 ...
 */
int
citadel_bcrypt_hashpass(const char *key, const char *salt, char *encrypted,
                      size_t encryptedlen)
{
    blf_ctx state;
    ...
    citadel_Blowfish_expandstate(&state, csalt, salt_len,
                               (u_int8_t *) key, key_len);
    for (k = 0; k < rounds; k++) {
        citadel_Blowfish_expand0state(&state, (u_int8_t *) key, key_len);
        citadel_Blowfish_expand0state(&state, csalt, salt_len);
    }

    /* This can be precomputed later */
    j = 0;
    for (i = 0; i < BCRYPT_WORDS; i++)
        cdata[i] = citadel_Blowfish_stream2word(ciphertext, 4 * BCRYPT_WORDS, &j);
    ...
    citadel_encode_base64(encrypted + 7, csalt, BCRYPT_MAXSALT);
    citadel_encode_base64(encrypted + 7 + 22, ciphertext, 4 * BCRYPT_WORDS - 1);
 ...
 */
int
citadel_encode_base64(char *b64buffer, const u_int8_t *data, size_t len)
{
